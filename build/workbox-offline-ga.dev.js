...
this.workbox.googleAnalytics = (function (exports, BackgroundSyncPlugin_js, cacheNames_js, getFriendlyURL_js, logger_js, Route_js, Router_js, NetworkFirst_js, NetworkOnly_js) {
    ...
    try {
      self['workbox:google-analytics:6.5.0'] && _();
    } catch (e) {}
      ...
      return async ({
        ...
        while (entry = await queue.shiftRequest()) {
          ...
          try {
            ...
            const params = request.method === 'POST' && request.bodyUsed ? new URLSearchParams(await request.clone().text()) : url.searchParams; // Calculate the qt param, accounting for the fact that an existing
